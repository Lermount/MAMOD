<template>
    <form id="form">
        <!--По идеи каждый элемент формы нужно вынести в отдельный компонент, но делать этого мы, конечно не будем-->
        <div id="name">
            <input type="text" placeholder="Имя" v-model="username" class="form-control"
                :class="{ 'attention': usernameError }">
            <span v-show="usernameError" class="error">Введите имя</span>
        </div>

        <div id="email">
            <input type="email" placeholder="Email" v-model="email" class="form-control"
                :class="{ 'attention': emailError }">
            <span v-show="emailError" class="error">Введите Email</span>
        </div>

        <div id="jobTitle">
            <select name="jobs" v-model="role" class="form-control placeholder" :class="{ 'attention': roleError }">
                <option :value="null" disabled hidden>Должность</option>
                <option class="job" v-for="job in jobItems" :key="job.value" :value="job.value">{{ job.name }}</option>
            </select>
            <i class="jobTitle-icon"></i>
            <span v-show="roleError" class="error">Выберите из списка</span>
        </div>

        <div id="password">
            <input :type="passwordIsVisible" placeholder="Пароль" v-model="password" class="form-control"
                :class="{ 'attention': passwordError }">
            <i class="password-icon" :class="{ 'passwordVisible': passwordIsVisible === 'text' }"
                @click="showPassword"></i>
            <span v-show="passwordError" class="error">Заполните это поле</span>
        </div>

        <div id="password_repeat">
            <input :type="passwordRepeatIsVisible" placeholder="Повторите пароль" :disabled="disabled"
                v-model="password_repeat" class="form-control" :class="{ 'attention': passwordRepeatError }">
            <i class="password-icon" :class="{ 'passwordVisible': passwordRepeatIsVisible === 'text' }"
                @click="showPasswordRepeat"></i>
            <span class="error" v-show="passwordRepeatError">Пароли не совпадают</span>
        </div>
    </form>
</template>
<script>
...
export default {
    props: ['usernameError', 'emailError', 'roleError', 'passwordError', 'passwordRepeatError'],
    ...
    data: () => ({
        ...
        passwordRepeatIsVisible: 'password',
        disabled: true
    }),

    watch: {
        //Эмитим данные формы в ролительский компонент(App.vue),
        //что бы от туда передать эти данные в функцию отправки формы в компоненте chek-app.vue
        //Потому что решил не устанавливать vuex
        username() {
        ...
        //Проверка на совпадение паролей из полей password и password_repeat
        //И т.к. приложенька плохо работала если сначала заполнить password_repeat, 
        //То просто заблокирвали возможность в инпут password_repeat пока не заполните password
        //Если password !== password_repeat кнопка сабмита формы будет disabled
        password() {
            this.$emit('updatePassword', this.password)
            this.disabled = false
            if (this.password == this.password_repeat) {
                this.$emit('updatePasswordRepeatError', false);
            } else {
                this.$emit('updatePasswordRepeatError', true);
            }
        },
        //Что бы корректно работало, если ввести пароль в оба поля, очистить и ввести сначала password_repeat
        password_repeat() {
            if (this.password_repeat == this.password) {
                this.$emit('updatePassword_repeat', this.password_repeat);
                this.$emit('updatePasswordRepeatError', false);
            } else {
                this.$emit('updatePasswordRepeatError', true);
            }
    ...
    computed: {
        //Получаем список профессий из заранее подготовленного JS файла
        jobItems() {
            return jobs
        }
    },
    ...
    methods: {
        //Показываем или скрываем пароли
        showPassword() {
        ...
        showPasswordRepeat() {
            if (this.passwordRepeatIsVisible === 'password') {
                this.passwordRepeatIsVisible = 'text'
            } else {
                this.passwordRepeatIsVisible = 'password'
            }
        },
    },
...
.jobTitle-icon {
    ...
    background-image: url("data:image/svg+xml,%3Csvg width='9' height='5' viewBox='0 0 9 5' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M8 1L4.5 4L1 1' stroke='%23A7A7B6' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'/%3E%3C/svg%3E%0A");
}
